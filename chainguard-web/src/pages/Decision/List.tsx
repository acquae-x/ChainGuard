    ...
    <PageContainer title="方案列表" extra={<Button type="primary" icon={<PlusOutlined />} onClick={() => history.push('/incident/list')}>从事件生成方案</Button>}>
      <ProTable<API.Proposal>
        ...
        columns={[
          ...
          { title: '总成本', dataIndex: 'totalCost', search: false, render: (_, row) => <SensitiveField field="cost" value={`¥${row.totalCost.toLocaleString()}`} /> },
          { title: '交期影响', dataIndex: 'leadTimeImpact', search: false, renderText: (value) => `${value} 天` },
          { title: '剩余风险', dataIndex: 'residualRisk', valueType: 'select', render: (_, row) => <RiskTag level={row.residualRisk} /> },
