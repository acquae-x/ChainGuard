declare namespace API {
  ...
  type User = { id: string; tenantId: string; name: string; phone: string; email?: string; deptId: string; roleIds: string[]; roleCode: RoleCode; status: 'active' | 'invited' | 'disabled'; permissions: string[]; dataScope: 'all' | 'dept' | 'custom'; readonly?: boolean };
  ...
  type Proposal = { id: string; incidentId: string; name: string; tag: 'recommended' | 'alternative' | 'invalid'; totalCost: number; leadTimeImpact: number; residualRisk: 'high' | 'medium' | 'low'; customerImpact: number; highValueCustomers: number; reason: string; views: Record<string, string>; modified?: boolean };
  type Approval = { id: string; proposalId: string; incidentId: string; status: string; riskLevel: string; summary: string; costImpact: number; submitter: string; waitingHours: number };
