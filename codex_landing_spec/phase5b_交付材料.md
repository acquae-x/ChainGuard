...
## Phase 5B 中文 OCR 产品鲁棒性修复（2026-07-19）

本节是对上一节产品界面闭环的独立复验修正，只处理本地图片 OCR、字段映射页和 API 模式
产品闭环；没有进入校准治理 UI、E-3、C3 或 A04，也没有删除/回退既有 OCR、C1 和映射逻辑。

### 根因与修复

- 逐像素检查历史失败 PNG 后确认：所谓“中文”区域实际已经在图片生成阶段变成了
  `????,????,??` 和连续问号，图片中不存在可供 OCR 恢复的中文字形。RapidOCR 对问号像素给出
  高置信度并如实转写；旧实现只检查文本框置信度，没有检查乱码和表格结构，因此把高置信度
  问号文本当成可预检内容。这是样本 Unicode 丢失和质量闸门缺失叠加导致的假绿灯，不是通过
  调低阈值可修复的问题。
- RapidOCR 子进程现在同时返回文本、置信度和检测框；服务端按纵向基线分组、组内从左到右排序，
  当引擎把同一行拆成多个框时用列分隔符重建，不再把每个检测框错误地当成独立数据行。
- 半角/全角逗号统一为 CSV 列分隔符；成功结果记录 `column_count`。真实微软雅黑小字号全角逗号
  样本可稳定形成 `物料编码`、`物料名称`、`成本` 三列。
- 新增独立于置信度阈值的质量闸门：连续问号/替换字符/典型异常编码返回
  `OCR_GARBLED_TEXT`；少于两行、少于两列、行间列数不一致或空白列返回
  `OCR_STRUCTURE_INCOMPLETE`。两类结果都保持 `manual_required`，不能 confirm/execute。
- API 将真实 `error_code`、中文原因和“重新上传清晰图片 / 改用 CSV/Excel 或人工录入”建议返回
  页面。字段映射页显示红色阻断原因和建议，人工确认复选框禁用，不再显示成“预检通过”。
- 默认最低置信度仍为 `0.75`；低置信度、空白、损坏、超时、引擎缺失的既有安全降级全部保留。

### 真实样本矩阵与实际结果

以下均调用本机 `rapidocr 3.9.1 + onnxruntime 1.27.0` CPU 模型，没有 mock OCR 输出。
耗时包含每张图片独立子进程和模型冷启动。

| 样本 | 格式/生成方式 | 实际结果 | 最低置信度 | OCR 耗时 |
|---|---|---|---:|---:|
| 中文表头、中文值 | PNG，Pillow + Windows 微软雅黑 48px | 2 行 × 3 列，字段和值完整 | 0.99632 | 3.902s |
| 中英文混合 | JPG，Pillow + Windows 微软雅黑 48px | 2 行 × 3 列，字段和值完整 | 0.99485 | 3.922s |
| 中文全角分隔符 | PNG，Pillow + Windows 微软雅黑 24px | 全角逗号规范化后 2 行 × 3 列 | 0.99969 | 4.064s |
| 中文产品闭环 | PNG，真实 Chromium 截图 | 三字段映射、成功 1/拒绝 0、物料页可见 | 0.99941 | 4.397s |
| 英文回归 | PNG，真实 Chromium 截图 | 三字段映射、成功 1/拒绝 0、物料页可见 | 0.99928 | 4.299s |
| 连续问号乱码 | PNG，真实渲染问号像素 | `OCR_GARBLED_TEXT`，页面阻断 | 高分文本仍阻断 | 4.182s |
| 无分隔符/结构损坏 | PNG，Pillow 真实渲染 | `OCR_STRUCTURE_INCOMPLETE`，无归一化数据 | 不以降阈值放行 | 实测通过 |
| 空白/损坏/低置信度 | PNG/JPG，真实文件 | 分别为 `OCR_EMPTY`、`OCR_IMAGE_DAMAGED`、`OCR_LOW_CONFIDENCE` | 保持 0.75 闸门 | 实测通过 |

其中 Chromium 中文用例完整执行：上传中文 PNG → 选择物料主数据 → 映射
`物料编码 → material_id`、`物料名称 → material_name`、`成本 → standard_cost` → 勾选人工确认
→ confirm/execute/job 轮询 → `succeeded` → 成功 1、拒绝 0 → 物料资料页可见
`MAT-UI-ZH-* / 中文界面验收芯片 / 12.50`。同一真实 API 模式用例随后上传问号乱码图，确认
页面显示原因和两条处理建议且确认控件不可用。

### 本轮实际验证

```text
后端 OCR/C1/预检/Phase 5B 相关回归：40 passed
后端真实 OCR 单文件矩阵：9 passed（含检测框行重建单测），无 mock OCR 通过声明
前端 TypeScript：npx tsc --noEmit，exit code 0
前端 Vitest 全量：11 files / 25 tests passed
DATA_MODE=api 生产构建：Webpack compiled successfully
Playwright API 模式真实 Chromium：1 passed（中文成功闭环 + 英文回归 + 乱码 UI 阻断）
```

真实 Chromium 验收使用迁移到 Alembic head 的 GUID 隔离 SQLite 数据库及独立 8410/8411
API/Web 端口。当前边界仍包括扫描型 PDF 栅格化、复杂合并单元格、严重倾斜/噪声、手写体和
高并发 OCR worker 池；这些场景继续安全转人工，不在本轮冒充完成。
