...
def render_sidebar() -> tuple[str, str | None, DataSource]:
    with st.sidebar:
        ...
        st.caption(
            "📥 企业数据导入已统一到主界面「🛠 一线从业者 → 数据接入 Agent」，"
            "支持 CSV/Excel/PDF/图片 多格式与杂数据；导入后回此处选择租户开始决策。"
        )
...
def render_data_intake(data_source, *, view: str = "一线从业者") -> None:
    """统一数据接入：多格式（CSV/Excel/PDF/图片 + OCR 级联）→ 归一化 → 容量预检 → 落库校验 → 导入复核。

    单一入口，取代旧的「侧栏 CSV 导入」与「独立接入面板」两处入口：
    规整的 ERP/WMS/TMS 导出与杂乱的现场单据都从这里进，统一处理。
    """
    import pathlib
    import tempfile
    ...
    st.markdown("**数据接入 Agent（多格式 + OCR 级联 · 统一入口）**")
    st.caption(
        "一个入口同时吃下规整的 ERP/WMS/TMS 导出和杂乱的现场单据："
        "CSV / Excel 直接读取，PDF / 图片走 LLM 视觉 → 离线 OCR → 文本层三级级联抽取，"
        "统一归一化后再做容量预检、落库冒烟校验与重复度复核。"
    )

    tenant_id = st.text_input("租户 ID（字母数字/_/-）", key="intake_tenant_id")
    server_dir = st.text_input(
        "服务器端目录路径（大批量 CSV 推荐，留空则用下方上传文件）",
        key="intake_server_dir",
    )
    ...
    has_input = bool(server_dir) or bool(uploads)
    ready = bool(tenant_id) and has_input
    if not ready:
        st.caption("填写租户 ID 并提供服务器目录或上传文件后，下方「先做容量预检 / 导入并校验」即可点击。")

    def _materialize_csv_dir(tmp_dir: str):
        """把输入统一成一个只含 CSV 的目录；返回 (csv_dir, ingestion_result|None)。"""
        if server_dir:
            return server_dir, None
        raw_paths: list[Path] = []
        for upload in uploads or []:
            raw = Path(tmp_dir) / "_raw" / Path(upload.name).name
            raw.parent.mkdir(parents=True, exist_ok=True)
            raw.write_bytes(upload.getbuffer())
            raw_paths.append(raw)
        result = ingest_files(raw_paths)
        for table_name, rows in result.normalized.items():
            _write_normalized_csv(Path(tmp_dir) / f"{table_name}.csv", rows)
        return tmp_dir, result

    def _show_ingestion(result) -> None:
        if result is None:
            return
        st.markdown("**多格式抽取报告**")
        ...
        st.caption(
            f"成功抽取 {result.ok_count} 个文件，"
            f"需人工录入/复核 {result.needs_manual_count} 个文件。"
        )
        for extraction in result.extractions:
            if extraction.needs_manual:
                st.warning(
                    f"{extraction.file_name} 需人工录入/复核：{extraction.note or '未识别'}"
                )
        for table_name, rows in result.normalized.items():
            st.caption(f"归一化预览 · {table_name}: {len(rows)} 行")
            st.dataframe(
    ...
    col_pre, col_imp = st.columns(2)
    do_preflight = col_pre.button(
        "先做容量预检", use_container_width=True, disabled=not ready
    )
    do_import = col_imp.button(
        "导入并校验", type="primary", use_container_width=True, disabled=not ready
    )

    if do_preflight:
        try:
            with tempfile.TemporaryDirectory() as tmp:
                csv_dir, result = _materialize_csv_dir(tmp)
                _show_ingestion(result)
                tenant = enterprise_source(tenant_id, require_exists=False).tenant_id
                db_path = tenant_scenario_db_path(tenant)
                csv_paths = sorted(pathlib.Path(csv_dir).glob("*.csv"))
                ...
                st.markdown(f"**容量预检结论：{verdict_cn}**")
                for msg in report.messages:
                    st.write(f"- {msg}")
                if report.verdict == "INSUFFICIENT_DISK":
                    st.error("磁盘空间不足，导入将被阻止，建议改用 PostgreSQL。")
                elif report.verdict == "REVIEW":
                    st.warning("数据量较大，建议评估 PostgreSQL 后再导入。")
                else:
                    st.success("容量预检通过，可执行导入。")
        except Exception as error:
            st.error(f"容量预检失败：{error}")
        return
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def _progress(table_name: str, rows: int) -> None:
                progress_bar.progress(
                    0.5, text=f"正在导入 {table_name}：累计 {rows:,} 行"
                )

            res = import_tenant_from_dir(tenant_id, csv_dir, progress=_progress)
            progress_bar.progress(1.0, text="导入完成")
            if res.preflight:
                st.caption(
                    f"容量预检：{res.preflight.get('verdict', '-')}；"
                    f"CSV {res.csv_rows:,} 行 / 落库 {res.db_rows:,} 行，"
                    f"对账{'一致' if res.reconciled else '存在差异'}。"
                )
            st.dataframe(res.table_results, hide_index=True, use_container_width=True)
            if not res.smoke_ok:
                st.error(f"❌ {res.smoke_message}（该租户未激活，请检查数据）")
                return
            st.success(f"✅ 导入成功（{res.ok_tables} 表）：{res.smoke_message}")

            review_csv = pathlib.Path(csv_dir) / "disruption_events.csv"
            ...
            with review_csv.open(encoding="utf-8-sig", newline="") as handle:
                review_records = list(_csv.DictReader(handle))
            from src.signature_history import load_history

            target_ds = enterprise_source(res.tenant_id)
            history = load_history(target_ds)
            render_intake_review(review_records, history, data_source=target_ds)
            st.caption("导入完成后，可在左侧「企业真实场景」选择该租户开始决策。")
    except Exception as error:
